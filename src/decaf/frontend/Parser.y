...
CaseExpr        :   CASE '(' Expr ')' '{' CaseList DefaultExpr '}'
                    {
                        $$.expr = new Tree.Case($3.expr, $6.slist, $7.expr, $1.loc);
                    }
                ;
CaseList        :   CaseList  ACaseEXor { $$.slist.add($2.stmt); }
                |   ACaseEXor 
                    {
                        $$.slist = new ArrayList<Tree>();
                        $$.slist.add($1.stmt);
...
ACaseEXor       :   Constant ':' Expr ';'
                    {
                        $$.stmt = new Tree.CaseItem($1.expr, $3.expr, $1.loc);
                    }
                ;
...
DoStmt          :   DO DoBranchList DoSubStmt OD
                    {
                        $2.slist.add($3.stmt);
                        $$.stmt = new Tree.Dood($2.slist, $1.loc);
                    }
                |   DO DoSubStmt OD
                    {
                        $2.slist = new ArrayList<Tree>();
                        $2.slist.add($2.stmt);
                        $$.stmt = new Tree.Dood($2.slist, $1.loc);
                    }
                ;
...
DoSubStmt       :   Expr ':' Stmt 
                    {
                        $$.stmt = new Tree.DoodBranch($1.expr, $3.stmt, $1.loc);
                    }
                    ;
