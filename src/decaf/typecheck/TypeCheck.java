...
import decaf.error.ConditionNotUniqueError;
import decaf.error.CaseMultiTypeError;
import decaf.error.SuperInStaticFuncError;
import decaf.error.CopyExprNotClassError;
import decaf.error.CopyExprIncompatError;
import decaf.error.DoStmtCondTypeError;
import decaf.error.BadPrintCompArgError;
import decaf.error.SuperMemberError;
...
public class TypeCheck extends Tree.Visitor {
	...
	public void visitBinary(Tree.Binary expr) {
		//改变了accept的顺序，便于插入#节点
		//只有在+ *混合运算时才改变int的类型
		expr.left.accept(this);
		expr.right.accept(this);
		if (expr.tag == Tree.PLUS || expr.tag == Tree.MUL) {
			if (expr.left.type == BaseType.COMPLEX && expr.right.type == BaseType.INT) {
				expr.right = new Tree.Unary(Tree.COMPCAST, expr.right, expr.right.loc);
				expr.right.type = BaseType.COMPLEX;
			}
				
			if (expr.right.type == BaseType.COMPLEX && expr.left.type == BaseType.INT) {
	...
	public void visitUnary(Tree.Unary expr) {
		...
		/* 新增部分判断 */
		else if (expr.tag == Tree.RE || expr.tag == Tree.IM) {
			if (expr.expr.type.equal(BaseType.ERROR)) {
				expr.type = expr.expr.type;
	...
	private void checkCallExpr(Tree.CallExpr callExpr) {
		ClassScope cs;
		Symbol f;
		if (callExpr.receiver == null) {
			cs = (ClassScope) table.lookForScope(Kind.CLASS);
			f = cs.lookupVisible(callExpr.method);
		}
			
		else if (!callExpr.receiver.type.equal(BaseType.ERROR) && callExpr.receiver instanceof Tree.Super) {
			ClassType parent = ((ClassType) callExpr.receiver.type).getParentType();
			if (parent == null) {
				issueError(new NoSuperParentError(callExpr.getLocation(), callExpr.receiver.type.toString()));
				callExpr.type = BaseType.ERROR;
				return;
			}
			f = null;
			while (parent != null) {
				cs = parent.getClassScope();
				if ((f = cs.lookupVisible(callExpr.method)) != null) {
					break;
				}
		...
		if (f == null) {
			issueError(new FieldNotFoundError(callExpr.getLocation(), callExpr.method, 
	...
	public void visitIdent(Tree.Ident ident) {
		if (ident.owner == null) {
			...
			if (!ident.owner.type.equal(BaseType.ERROR)) {
				if (ident.owner instanceof Tree.Super) {
					issueError(new SuperMemberError(ident.getLocation()));
					ident.type = BaseType.ERROR;
					return;
				}
	...
	public void visitAssign(Tree.Assign assign) {
		...
		if (assign.expr instanceof Tree.Dcopy || assign.expr instanceof Tree.Scopy) {
			if (!assign.left.type.equal(assign.expr.type) 
				&& !assign.expr.type.equal(BaseType.ERROR) 
				&& !assign.left.type.equal(BaseType.ERROR))
				issueError(new CopyExprIncompatError(assign.getLocation(),
					assign.expr.type.toString(), assign.left.type
							.toString()));
			return;
		}
	...
	private Type checkBinaryOp(Tree.Expr left, Tree.Expr right, int op, Location location) {
		...
		case Tree.PLUS:
		case Tree.MUL:
			if (left.type == right.type && left.type == BaseType.COMPLEX) {
				compatible = true;
	...
	public void visitPrintComp(Tree.PrintComp printcompStmt) {
		...
		for (Tree.Expr e : printcompStmt.exprs) {
			e.accept(this);
			i++;
			if (!e.type.equal(BaseType.COMPLEX) && !e.type.equal(BaseType.ERROR)) {
	...
	public void visitCase(Tree.Case caseExpr) {
		...
		if (!caseExpr.condition.type.equal(BaseType.ERROR) && !caseExpr.condition.type.equal(BaseType.INT))
			issueError(new IncompatCaseExprError(caseExpr.condition.getLocation(), caseExpr.condition.type.toString()));
		HashSet<Integer> conds = new HashSet<>();
		Type t = caseExpr.defaultExpr.type;
		boolean alignExpr = true;
		for (Tree var : caseExpr.caseList) {
			Tree.CaseItem e = (Tree.CaseItem)var;
			e.accept(this);
			if (e.cond.type == BaseType.INT) {
				if (conds.contains((Integer)e.cond.value)) 
					issueError(new ConditionNotUniqueError(e.cond.getLocation()));
				else conds.add((Integer)e.cond.value);
			}
			if (!t.equal(e.expr.type) && !e.expr.type.equal(BaseType.ERROR)) {
				alignExpr = false;
				issueError(new CaseMultiTypeError(e.cond.getLocation(), e.expr.type.toString(), t.toString()));
				
			}
		}
			
		if (alignExpr) caseExpr.type = t;
		else caseExpr.type = BaseType.ERROR;
	}
	@Override
	public void visitCaseItem(Tree.CaseItem caseitem) {
		caseitem.cond.accept(this);
		if (!caseitem.cond.type.equal(BaseType.INT) && !caseitem.cond.type.equal(BaseType.ERROR))//避免多报错
			issueError(new IncompatCaseExprError(caseitem.cond.getLocation(), caseitem.cond.type.toString()));
		caseitem.expr.accept(this);
		caseitem.type = caseitem.expr.type;
	}
	@Override
	...
	public void visitScopy(Tree.Scopy scopyExpr) {
		...
		if (scopyExpr.expr.type.isClassType() || scopyExpr.expr.type.equal(BaseType.ERROR))
			...
			issueError(new CopyExprNotClassError(scopyExpr.getLocation(), scopyExpr.expr.type.toString()));
		}
	...
	public void visitDcopy(Tree.Dcopy dcopyExpr) {
		...
		if (dcopyExpr.expr.type.isClassType() || dcopyExpr.expr.type.equal(BaseType.ERROR))
			...
			issueError(new CopyExprNotClassError(dcopyExpr.getLocation(), dcopyExpr.expr.type.toString()));
		}
			
	}
	@Override
	public void visitDood(Tree.Dood dood) {
		breaks.add(dood);
        for (Tree tree : dood.branches) {
        	Tree.DoodBranch branch = (Tree.DoodBranch) tree;
        	branch.accept(this);
        }
        breaks.pop();
    }
    @Override
    public void visitDoodBranch(Tree.DoodBranch doodbranch) {
        doodbranch.expr.accept(this);
        //issueError(new DoStmtCondTypeError(doodbranch.expr.getLocation(), doodbranch.expr.type.toString()));
        doodbranch.stmt.accept(this);
        if (!doodbranch.expr.type.equal(BaseType.BOOL) && !doodbranch.expr.type.equal(BaseType.ERROR))
			issueError(new DoStmtCondTypeError(doodbranch.expr.getLocation(), doodbranch.expr.type.toString()));
    }
